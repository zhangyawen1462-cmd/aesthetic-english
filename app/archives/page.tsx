...
// --- 1. 做旧质感 (Aged Paper Grain) ---
// 降低 baseFrequency (颗粒变大)，提高 opacity (更明显)，模拟粗糙旧纸
...
export default function VintageArchive() {
  ...
  return (
    ...
    <div className="min-h-screen w-full bg-[#E6E0D4] selection:bg-[#2b0b0b] selection:text-[#E6E0D4] relative overflow-hidden font-sans">
      ...
      {/* --- B. 悬挂式书签 (The Hanging Bookmark) --- */}
      {/* 替代了原来的 Header 按钮，这是一个物理书签设计 */}
      ...
      {/* --- C. 做旧纹理层 (Aged Texture) --- */}
      {/* 混合模式 multiply + sepia 滤镜，制造岁月感 */}
      <div 
    ...
    >
      <Link href={`/course/${item.category}/${item.id}`} className="group block h-full perspective-1000">
        ...
        `}>
          ...
          {/* --- 会呼吸的磨砂层 (The Breathing Frost) --- */}
          <div 
            className={`absolute inset-0 z-10 backdrop-blur-[2px] transition-all duration-1000 ease-in-out group-hover:opacity-0 group-hover:backdrop-blur-none ${item.overlayColor}`} 
            style={{ opacity: 0.8 }} 
          />
